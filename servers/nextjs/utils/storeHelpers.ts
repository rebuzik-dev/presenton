...
export const normalizeLLMConfig = (llmConfig: LLMConfig): LLMConfig => {
  ...
  if (
    ...
    normalizedConfig.IMAGE_GEN_BASE_URL &&
    normalizedConfig.IMAGE_GEN_API_KEY &&
    normalizedConfig.IMAGE_GEN_MODEL
  ) {
    normalizedConfig.IMAGE_PROVIDER = "custom_openai";
    ...
    switch (llmConfig.IMAGE_PROVIDER) {
      ...
      case "polza":
        if (
          ...
          !isProvided(llmConfig.IMAGE_GEN_MODEL?.trim())
        ) {
          return "Custom image API requires base URL, API key, and model.";
        }
        break;
      default:
