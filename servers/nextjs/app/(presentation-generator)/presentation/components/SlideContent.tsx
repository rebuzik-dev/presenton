...
interface SlideContentProps {
  ...
  onRequestBlockEdit?: (slideIndex: number) => void;
}
        ...
        >
          ...
          {!isStreaming && (
            <div className="absolute top-2 z-20 sm:top-4 hidden md:block left-2 sm:left-4 transition-transform">
              <ToolTip content="Edit block prompts">
                <button
                  type="button"
                  className="rounded-lg bg-[#5141e5] p-2 shadow-md transition-all duration-300 hover:shadow-md group-hover:scale-105"
                  onClick={() => {
                    trackEvent(MixpanelEvent.Slide_Update_From_Prompt_Button_Clicked, { pathname });
                    onRequestBlockEdit?.(slide.index);
                  }}
                >
                  <WandSparkles className="h-4 w-4 text-white sm:h-5 sm:w-5" />
                </button>
              </ToolTip>
            </div>
          )}
