...
const layoutDescription =
  "Концепция подарочного набора: основная идея, задача для получателя и принципы подбора взаимосвязанных компонентов на основе брифа."

const Schema = z.object({
  title: z.string().min(5).max(40).default("КОНЦЕПЦИЯ НАБОРА").meta({ description: "Заголовок раздела о концепции подарочного набора." }),
  quote: z
    .string()
    .min(20)
    .max(220)
    .default(
      "Подарочный набор продолжает идею мероприятия и объединяет уместные для получателя компоненты в цельный сценарий вручения и распаковки."
    )
    .meta({ description: "Одна ёмкая идея линейки: какую связь с мероприятием и ценность для получателя она создаёт." }),
  leftTitle: z.string().min(3).max(20).default("Задача").meta({ description: "Заголовок блока с назначением подарочного набора." }),
  ...
  rightBody: z
    ...
    .max(240)
    .default(
      "Компоненты объединяются общей визуальной системой, материалами, упаковкой и логикой взаимодействия; конкретные позиции берутся из брифа или предлагаются как варианты."
    )
    .meta({ description: "Критерии выбора компонентов: уместность для получателя, совместимость, качество, бюджет, производство, упаковка и брендинг в рамках брифа." }),
})
      ...
      <div className="h-full px-16 pt-12 pb-10 flex flex-col">
        ...
        <div className="mt-10 rounded-[18px] bg-[#8F9499] shadow-[0_12px_18px_rgba(0,0,0,0.20)] px-14 py-10 flex gap-10 items-start" style={{ backgroundColor: quoteBackground }}>
          ...
          <div className="text-[26px] leading-[34px] font-[500] text-white overflow-hidden max-w-[980px]" style={{ color: quoteTextColor, fontFamily: quoteFont }}>
            ...
            >
              {slideData?.quote ||
                "Подарочный набор продолжает идею мероприятия и объединяет уместные компоненты в цельный сценарий вручения."}
            </span>
          </div>
        </div>
        ...
        <div className="mt-14 grid grid-cols-2 gap-20 flex-1 min-h-0">
          ...
          <div className="min-h-0">
            ...
            <div className="mt-8 text-[22px] leading-[30px] font-[400] text-[var(--style-text-primary)] overflow-hidden max-w-[520px]" style={{ color: bodyColor, fontFamily: bodyFont }}>
              ...
              >
                {slideData?.rightBody ||
                  "Компоненты объединяются общей визуальной системой, материалами, упаковкой и логикой взаимодействия."}
              </span>
            </div>
          </div>
        </div>
