...
const ImageSchema = z.object({
  ...
  __image_prompt__: z
    ...
    .min(3)
    .max(180)
    .default("Абстрактный фон, передающий общий визуальный код мероприятия без текста и случайных логотипов")
...
const IconSchema = z.object({
  ...
  __icon_prompt__: z
    ...
    .min(1)
    .max(60)
    .default("нейтральный символ визуальной системы мероприятия")
...
const layoutDescription =
  "Обложка общего визуального кода: тип документа и точное название мероприятия из брифа, без вымышленных дат, площадок и форматов."
...
const Schema = z.object({
  kicker: z
    ...
    .min(3)
    .max(40)
    .default("Общий визуальный код")
    .meta({ description: "Тип документа: общий визуальный код или формулировка из брифа. До 4 слов." }),
  title: z
    .string()
    .min(5)
    .max(55)
  ...
  return (
    <div className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-white relative z-20 mx-auto overflow-hidden" style={resolveRootStyle(slideData, "#FFFFFF", "var(--template-font, Inter)")}>
      ...
      <div className="relative h-full px-20 pt-44">
        ...
        >
          {slideData?.kicker || "Общий визуальный код"}
        </div>
