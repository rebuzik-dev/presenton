...
class IconFinderService:
    ...
    def _initialize_icons_collection(self):
        ...
        try:
            ...
            print(
                "[IconFinder] Bundled vectorstore exists: "
                f"{os.path.isfile(bundled_vectorstore_path)}"
            )
            print(
                "[IconFinder] Writable vectorstore exists: "
                f"{os.path.isfile(writable_vectorstore_path)}"
            )
            ...
            if os.path.isfile(bundled_vectorstore_path):
                ...
                print(
                    "[IconFinder] Loading vectorstore from bundled location: "
                    f"{vectorstore_path}"
                )
            elif os.path.isfile(writable_vectorstore_path):
                vectorstore_path = writable_vectorstore_path
                print(
                    "[IconFinder] Loading vectorstore from writable location: "
                    f"{vectorstore_path}"
                )
            ...
            if vectorstore_path:
                ...
                if documents:
                    ...
                    if success:
                        print(
                            f"[IconFinder] Successfully embedded {len(documents)} icons"
                        )
                        try:
                            os.makedirs(
                                os.path.dirname(writable_vectorstore_path),
                                exist_ok=True,
                            )
                            self.vectorstore.save(writable_vectorstore_path)
                            print(
                                "[IconFinder] Vectorstore saved to "
                                f"{writable_vectorstore_path}"
                            )
                        except Exception as e:
